...
```{r}
data<-read_excel("RecapTM2S20202021.xlsx")
head(data)
...
```{r}
data<-read_excel("TM2S2022.xlsx")
head(data)
exos_asy=data[,c(4,5,6,8,9,10,12,14,15,17,18)]
M<-cor(exos_asy)
M
corrplot(M,method='circle')
#changer les oui/non en O/1 pour voir !!!
```
Here, we can see the correlation between Mobinfdroite and Mobinfgauche. That is what we were expecting. We can also see that the categories are not really linked (or correlated). The different exercises are not repetitive and each one is playing a distincly important role. 
```{r}
...
Ind
```
We notice that the p value is higher than 0.05 (5%). We can conclude that the sport and the asymetry are related, they are not independent.
```{r}
